...
MainTabClass.prototype.drawInterface = function() {
    $(this.selector).append( "<div class='tab-pane active panel panel-info withpadding' id='tab_main'>" +
        ...
        "<p><button type='button' class='btn btn-default btn-block' id='tokenbutton'>get Token</button></p>" +
        "<p>Token-ID<input type='text' class='form-control' id='tokenid' placeholder='enter token ID'></p>" +
        "</div>" );
...
MainTabClass.prototype.isReady = function() {
    $( "#tokenbutton" ).click(function() {
        $.AjaxRequest({action: 'gettoken' },function (msg) {
            MyLogger.info(msg);
            $("#tokenid").val(msg);
        });
    });
