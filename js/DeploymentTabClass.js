...
DeploymentTabClass.prototype.isReady = function() {
    ...
    $( "#createinstancebutton" ).click(function() {
        ...
        $.each($.creationVariables, function (i, item) {
            ...
            if( found===false ) {
                requestData[item.name] = value;
            }
        });

        requestData['action'] = 'createinstance';
        //requestData['tenant'] = $("#tenantid").val();
        requestData['url'] = $.haas.serviceURL;
        requestData['oterm'] = $("#oterm").val();

        $.AjaxRequest(JSON.stringify(requestData));
    });
